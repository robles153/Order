...
namespace Order.Domain.Services
{
    public class ClientService : IClientService
    {
        ...
        async Task<Response> IClientService.DeleteAsync(string clientId)
        {
            ...
            if (!exists)
            {
                response.Reports.Add(Report.Create($"Client {clientId} not exists!"));
                return response;
            }
       ...
       async  Task<Response<List<ClientModel>>> IClientService.ListByFilterssync(string clientId, string name)
        {
        ...
        async Task<Response> IClientService.UpdateAsync(ClientModel client)
        {
           ...
           var exists = await _clientRepository.ExistByIdAsync(client.Id);
            ...
            if (!exists)
            {
                response.Reports.Add(Report.Create($"Client {client.Id} not exists!"));
                return response;
            }
